...
impl Directory {
  ...
  pub fn fold(&mut self) -> Option<File> {
    ...
    // if let File::SymLink(symlink) = first && entries.all(|entry| {
    if let File::SymLink(symlink) = first {
      let same_parents = entries.all(|entry| {
        if let File::SymLink(symlink2) = entry {
          symlink.target.parent() == symlink2.target.parent()
        } else {
          false
        }
      });
      ...
      if same_parents {
        symlink.target.pop();
        return Some(File::symlink(symlink.target.take()));
      }
    }
